...
+!addLiteralObligationEndCondition(false, Obligation, Options) : true
   <- .puts("Obligation to ", Obligation, " never ends.").
...
+?findPlansWithAction(Prohibition, [Plan | Plans], [Plan | SelectedPlans]) 
...
+?findPlansWithEffect(Prohibition, [Plan | Plans], [Plan | SelectedPlans]) 
...
+!suppressPlan(Plan) : org.kcl.iovis.reflect.plan_label(Plan, Label)
	<- .print("Suppressing plan labelled '",Label,"'");
	   .remove_plan(Label).
...
+!unsuppressPlan(Plan) : true
	<- .print("Unsuppressing Plan, ",Plan);
	   .add_plan(Plan).